...
async function migrate() {
  ...
  await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS role TEXT DEFAULT 'propietario';`)
  ...
  await pool.query(`
    ...
    SELECT * FROM (VALUES
      ('Base',     1, 'Ideal para empezar con un único alojamiento.',                            2999),
      ('Esencial', 3, 'Para quienes tienen más de un alojamiento y quieren crecer.',             4999),
      ('Avanzado', NULL, 'Para operadores con múltiples propiedades. Sin límites.', 0)
    ) AS v(name, max_properties, description, cost)
    WHERE NOT EXISTS (SELECT 1 FROM plans);
  `)
  console.log('✅ Planes por defecto verificados.')
